...
pdf_text = read_pdf("C:\\Users\\pooja\\Downloads\\Big Mac Index.pdf")
print(pdf_text)
...
tokenizer_url = "https://huggingface.co/gpt2/resolve/main/vocab.json"


model = torch.hub.load_state_dict_from_url(model_url)
tokenizer = torch.hub.load_state_dict_from_url(tokenizer_url)
...
Six fastest earned (July 2015)
This statistic shows the average working time required to buy one Big Mac in selected cities around the world in 2015.
Hong Kong – 8.6 min
Luxembourg – 10.3 min
...
chain = LLMChain(prompt=prompt_text, llm=model, tokenizer=tokenizer)

# Generate questions
questions = chain.generate_questions(max_questions=5)
